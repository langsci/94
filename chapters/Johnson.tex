...
In this section, I show that the construction that I argue instantiates VPE in Hocąk displays the two defining characteristics of VPE. In \sectref{sec:johnson:2.1}, I demonstrate that the ellipsis site includes all VP-internal material. In \sectref{sec:johnson:2.2}, I show that VPE is subject to the presence of an appropriate licensing head.

\subsection{\emph{ųų} targets the VP}\label{sec:johnson:2.1}

VPE is possible with both intransitive and transitive verbs, as seen in (\ref{ex:johnson:3}-\ref{ex:johnson:4}) below. \REF{ex:johnson:3a} and \REF{ex:johnson:3b} show that \emph{ųų} can target intransitive VPs. In the examples in \REF{ex:johnson:4} with transitive verbs, the direct object is also included in the ellipsis site.
...
VPE can also target other internal arguments. Both indirect objects and resultative phrases are typically analyzed as VP-internal (see e.g., \citealt{Larson1988} and \citealt{LevinRappaportHovav1995}), and they are also subject to VPE. The ditransitive example in \REF{ex:johnson:5} shows that both a direct object and indirect object can be contained in the ellipsis site.
...
VPE also targets various adjuncts. \REF{ex:johnson:7} shows that VPE targets VPs containing temporal adjuncts. 
...
In \REF{ex:johnson:8}, locative adjuncts are included in the ellipsis site. 
...
\REF{ex:johnson:10} demonstrates that various manner adverbs can also be subject to VPE.
...
The main characteristic that distinguishes VPE from other elliptical processes is the presence of an overt licensing head located in the inflectional domain above the VP. VPE in English can be licensed by a variety of functional elements, such as \emph{do} in \REF{ex:johnson:12a}, \emph{be} in \REF{ex:johnson:12b}, \emph{have} in \REF{ex:johnson:12c}, \emph{can} in \REF{ex:johnson:12d} and \emph{will} in \REF{ex:johnson:12e}. The obligatory presence of an inflectional head has led previous researchers to argue that VPE is licensed by T/Infl \citep{Bresnan1976,Sag1976,Zagona1988,Lobeck1995}.
...
Tense and modals can be present in VPE constructions; however, they are never obligatory. When present, tense and modals always co-occur with the light verb \emph{ųų}. \REF{ex:johnson:19a} shows that the future tense marker \emph{kjane} can follow \emph{ųų}, while \REF{ex:johnson:19b} and \REF{ex:johnson:19c} demonstrate that the modals \emph{ną} and \emph{s'aare} can also appear after \emph{ųų}. When \emph{ųų} is omitted, the result is ungrammatical.
...
\citet{Goldberg2005} notes that English VPE is possible with a variety of sentence types. VPE is licit when the antecedent VP and elided VP are found in conjoined CPs \REF{ex:johnson:25a}, in adjacent CPs uttered by the same speaker \REF{ex:johnson:25b}, and when the antecedent is in a question and the ellipsis site in the answer \REF{ex:johnson:25c}. In this section, I show that the same is true in Hocąk.
...
All of the examples of VPE we saw in \sectref{sec:johnson:2} involved two clauses joined by the coordinator \emph{anąga} `and'. VPE is also possible with disjunction, as seen in \REF{ex:johnson:26} with \emph{nųnįge} `but'.
...
Lastly, VPE also occurs in question-answer pairs in Hocąk. In \REF{ex:johnson:28a}, a yes-no question contains the antecedent VP and the answer contains the gap. \REF{ex:johnson:28b} demonstrate that the same holds of \emph{wh}-questions.
...
\citet{Goldberg2005} distinguishes VPE from null complement anaphora (NCA) based on the type of constituent that is elided. In NCA, a matrix verb is stranded and its complement is elided. However, NCA is constrained by the contents of the VP: only propositions can be elided. This is illustrated by the contrast between the grammatical NCA examples in \REF{ex:johnson:29a} and \REF{ex:johnson:29c} and the ungrammatical examples in \REF{ex:johnson:29b} and \REF{ex:johnson:29d}:
...
As \citet{Fortin2007} points out, this diagnostic does not serve to distinguish VPE from NCA in languages with null objects. Hocąk allows both null subjects and objects, as seen in \REF{ex:johnson:31b}:
...
\citet{Goldberg2005} notes that the ellipsis site in VPE constructions can be inside an adjunct island, while gapping is not permitted in adjuncts. This is shown by the contrast between \REF{ex:johnson:34a} and \REF{ex:johnson:34b} below:
...
The same contrast is found in Hocąk. The examples in \REF{ex:johnson:35} show that the gap in VPE constructions can be found inside adjunct clauses (which precede the main clause in these examples). In \REF{ex:johnson:3a}, the ellipsis site is in the clause headed by `if', in \REF{ex:johnson:35b} the ellipsis site is in the clause headed by `because', and in \REF{ex:johnson:35c} it is in the clause headed by `before.'
...
In contrast, gapping is ungrammatical in adjuncts. \REF{ex:johnson:36} illustrates that the the gap cannot be located in an adjunct clause headed by `if' \REF{ex:johnson:36a}, `because' \REF{ex:johnson:36b} or `before' \REF{ex:johnson:36c}.
...
\citet{Goldberg2005} also shows that the ellipsis site in VPE constructions can be inside an embedded clause, while this is not true of other types of ellipsis. \REF{ex:johnson:37a} demonstrates that VPE is licit in an embedded clause, while \REF{ex:johnson:3b}--\REF{ex:johnson:3c} illustrate that neither gapping nor stripping are possible in an embedded clause.
...
In Hocąk, VPE is licit in the complement clause of various matrix verbs, including `know' \REF{ex:johnson:38a}, `want' \REF{ex:johnson:38b}, `think' \REF{ex:johnson:38c} and `say' \REF{ex:johnson:38d}.
...
Unlike English, Hocąk does not exhibit any constraint on gapping in embedded contexts. \REF{ex:johnson:39a} and \REF{ex:johnson:39b} show that the gap can be embedded under the verbs \emph{hire} `think' and \emph{ee} `say', respectively.
...
The examples in \REF{ex:johnson:40} show that Hocąk exhibits stripping. \REF{ex:johnson:40a} illustrates stripping with an object remnant after the coordinator `and', while the example in \REF{ex:johnson:40b} has an object remnant with disjunction. \REF{ex:johnson:40c} shows that stripping is also possible with a subject remnant after the coordinator.
...
As is the case in English and other languages, stripping is ungrammatical in embedded clauses in Hocąk. This is shown in \REF{ex:johnson:41a} for an object remnant with conjunction, \REF{ex:johnson:41b} for an object remnant with disjunction and \REF{ex:johnson:41c} for a subject remnant with conjunction.
...
\trans Intended: `Bryan didn't buy a car, but Cecil said (he bought) a bicycle.'}\label{ex:johnson:41b}
\ex[*] {
...
\trans Intended: `Sarah baked a cake, and Bryan said Meredith (baked a cake) too.'}\label{ex:johnson:41c}
\z
\z
...
In Hocąk, strict and sloppy readings are available with both VPE and stripping, while the additional ``object reading'' is possible only with stripping. In the examples in \REF{ex:johnson:44}, the antecedent VP contains a possessed object. \REF{ex:johnson:44a} is an instance of VPE, and the second conjunct has two possible interpretations: either Hunter visited Bryan's mother (strict reading) or Hunter visited his own mother (sloppy reading). In \REF{ex:johnson:44b}, the second conjunct contains a stripping ellipsis site. Both the strict and sloppy readings are available, but the object reading is also possible: the sentence could mean that Bryan visited Hunter.
...
The examples in \REF{ex:johnson:45} show that the same readings are possible with reflexives. The second conjunct of \REF{ex:johnson:45a} contains a VPE gap, and it has two interpretations: either Meredith hit Mateja (sloppy) or Meredith hit herself (strict). In the stripping example in \REF{ex:johnson:45b}, both strict and sloppy readings are possible, but so is the ``object reading'' under which Mateja hit Meredith.
...
Likewise, Hocąk constructions with \emph{ųų} cannot be analyzed as a \emph{pro}-form, as object extraction is permitted. \REF{ex:johnson:47a} shows that focused elements can be extracted from the ellipsis site, and \REF{ex:johnson:47b} exemplifies the movement of \emph{wh}-words from the ellipsis site.\footnote{Like other Siouan languages, Hocąk is a \emph{wh}-in-situ language. However, \emph{wh}-words can undergo focus driven movement.}
...
The second argument in favor of a deletion analysis of VPE in Hocąk comes from so-called ``missing antecedents.'' \citet{HankamerSag1976} demonstrate that the gap in English VPE constructions can contain the antecedent to a pronoun. In the non-elliptical example in \REF{ex:johnson:49a}, the DP \emph{a camel} in the second conjunct serves as the antecedent for the pronoun \emph{it} in the third conjunct. In \REF{ex:johnson:49b}, the VP in the second conjunct is elided, resulting in a missing antecedent for the pronoun \emph{it}. Nonetheless, the sentence is still grammatical. It is important to note that the instance of \emph{a camel} in the first conjunct cannot be the antecedent for the pronoun \emph{it}: as \REF{ex:johnson:50} shows, DPs under the scope of negation cannot serve as antecedents for pronouns.
...
\citet{HankamerSag1976} argue that the grammaticality of the example in \REF{ex:johnson:49b} points to a deletion analysis of VPE. These facts are not readily explained under a \emph{pro}-form analysis: since the ellipsis site would not have internal structure at any point in the derivation, the elided VP in \REF{ex:johnson:49b} would never contain the antecedent for the following pronoun. 

Examples of VPE with missing antecedents are also grammatical in Hocąk. In \REF{ex:johnson:51a}, the DP \emph{kšeexetehižą} `a pineapple' in the second conjunct is the antecedent for the null pronominal subject of the verb \emph{sguu} `sweet'. In \REF{ex:johnson:51b}, the VP containing the antecedent is elided, and the resulting sentence is grammatical. Like English, a pronoun cannot find its antecedent in a negated clause \REF{ex:johnson:52}.
...
First and foremost, I would like to thank \iai{Cecil Garvin} for sharing his language with me and kindly answering my endless questions. Thanks also to \iai{Yafei Li}, \iai{Bryan Rosen} and \iai{Rand Valentine} for discussion and comments at various stages of this project. A portion of this work was presented at the 87\textsuperscript{th} Annual Meeting of the Linguistic Society of America. I thank the audience there for questions and comments. I would also like to thank an anonymous reviewer and \iai{Catherine Rudin} for comments on an earlier draft of this paper. Lastly, thanks to \iai{Iren Hartmann} for generously providing me access to her Lexique Pro Hocąk dictionary to assist with my fieldwork.

...
\todo{check underline package}
\todo{ist the sc at lm* l intended?}
...
In example \REF{garvinphraseaccent}, \ili{Ho-Chunk} speaker Cecil Garvin\ia{Garvin, Cecil} uses a !H phrase accent on both the linking \emph{coowexjįšgera} `just a little' and the stage \isi{topic} \emph{karacgą ’ųnąą\-kaš\-ge\-ra} `since they were drinking'. On the other hand, the last two links -- \emph{coowera} `a little' and \emph{hoinąk haanįsge} `I started too' -- both of which are also coded as relational foci, receive L phrase accents:

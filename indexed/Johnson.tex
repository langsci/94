...
This purpose of this paper is to both argue that \il{Ho-Chunk}Hocąk displays \is{ellipsis, verb-phrase}verb-phrase ellipsis (\is{ellipsis, verb-phrase}VPE) and to provide the first thorough description of this phenomenon. In \is{ellipsis, verb-phrase}VPE constructions, a \is{verb phrase}VP goes unpronounced when there is an appropriate antecedent \is{verb phrase}VP and a \isi{licensing head} that identifies the gap. Both of these properties can be seen in the examples of \is{ellipsis, verb-phrase}VPE in \il{Ho-Chunk}Hocąk in \REF{ex:johnson:1} below. In each example, the \is{verb phrase}VP in the second conjunct is interpreted as identical to the \is{verb phrase}VP in the first conjunct, even though the former has no phonological realization. Instead, the \isi{light verb} \emph{ųų} takes the place of the \is{verb phrase}VP.
...
The main characteristic that distinguishes \is{ellipsis, verb-phrase}VPE from other elliptical processes is the presence of an overt \isi{licensing head} located in the inflectional domain above the \is{verb phrase}VP. \is{ellipsis, verb-phrase}VPE in \ili{English} can be licensed by a variety of functional elements, such as \emph{do} in \REF{ex:johnson:12a}, \emph{be} in \REF{ex:johnson:12b}, \emph{have} in \REF{ex:johnson:12c}, \emph{can} in \REF{ex:johnson:12d} and \emph{will} in \REF{ex:johnson:12e}. The obligatory presence of an inflectional head has led previous researchers to argue that \is{ellipsis, verb-phrase}VPE is licensed by T/Infl \citep{Bresnan1976,Sag1976,Zagona1988,Lobeck1995}.
...
To formalize this restriction on \is{ellipsis, verb-phrase}VPE in \il{Ho-Chunk}Hocąk, I adopt \citegen{Merchant2001} proposal that \isi{ellipsis} takes place when a so-called ``[E]-feature'' is present on the relevant \isi{licensing head}. In the case of \il{Ho-Chunk}Hocąk, I propose that an [E]-feature is present only on the agentive \emph{v} head.\footnote{This agentivity requirement on a process that affects the \is{verb phrase}VP is not completely unique to \il{Ho-Chunk}Hocąk. For example, \citet{Hallman2004} notes that \ili{English} \emph{do so} replacement is restricted to agentive VPs, even though other uses of \emph{do} are not subject to this constraint (e.g., \emph{Max loves studying \ili{French}, and Mary does (*so) too.}) \citet{Rouveret2012} also shows that \is{ellipsis, verb-phrase}VPE in \ili{Welsh} is licensed uniquely by the \isi{light verb} \emph{gweund}, and furthermore that \is{ellipsis, verb-phrase}VPE is not permitted with stative predicates. The only possibility with stative VPs is V\is{ellipsis, verb-phrase}VPE. However, Rouveret also shows that \emph{gweund} is also incompatible with stative predicates in its non-elliptical uses. This contrasts with the behavior of \emph{ųų} in \il{Ho-Chunk}Hocąk and \emph{do} in \emph{do so} in \ili{English}.}  This accounts for the fact that \is{ellipsis, verb-phrase}VPE is solely conditioned by the presence of the \isi{light verb} (or \emph{v}) \emph{ųų}, and furthermore that only agentive verbs can be elided: if there is a non-agentive \emph{v} present, then \isi{ellipsis} will not be licensed. 
...
All of the examples of \is{ellipsis, verb-phrase}VPE we saw in \sectref{sec:johnson:2} involved two \isi{clauses} joined by the coordinator \emph{anąga} `and'. \is{ellipsis, verb-phrase}VPE is also possible with disjunction, as seen in \REF{ex:johnson:26} with \emph{nųnįge} `but'.
...
\REF{ex:johnson:27} shows that \is{ellipsis, verb-phrase}VPE is also licit in adjacent CPs. In each example, the antecedent \is{verb phrase}VP is found in the first sentence while the \isi{ellipsis} site is in the second sentence.
...
\citet{Goldberg2005} notes that the \isi{ellipsis} site in \is{ellipsis, verb-phrase}VPE constructions can be inside an \isi{adjunct} island, while \isi{gapping} is not permitted in adjuncts. This is shown by the contrast between \REF{ex:johnson:34a} and \REF{ex:johnson:34b} below:
...
In \il{Ho-Chunk}Hocąk, strict and \isi{sloppy readings} are available with both \is{ellipsis, verb-phrase}VPE and \isi{stripping}, while the additional ``\isi{object} reading'' is possible only with \isi{stripping}. In the examples in \REF{ex:johnson:44}, the antecedent \is{verb phrase}VP contains a \isi{possessed} \isi{object}. \REF{ex:johnson:44a} is an instance of \is{ellipsis, verb-phrase}VPE, and the second conjunct has two possible interpretations: either Hunter visited Bryan's mother (strict reading) or Hunter visited his own mother (sloppy reading). In \REF{ex:johnson:44b}, the second conjunct contains a \isi{stripping} \isi{ellipsis} site. Both the strict and \isi{sloppy readings} are available, but the \isi{object} reading is also possible: the sentence could mean that Bryan visited Hunter.
...
The examples in \REF{ex:johnson:45} show that the same readings are possible with reflexives. The second conjunct of \REF{ex:johnson:45a} contains a \is{ellipsis, verb-phrase}VPE gap, and it has two interpretations: either Meredith hit Mateja (sloppy) or Meredith hit herself (strict). In the \isi{stripping} example in \REF{ex:johnson:45b}, both strict and \isi{sloppy readings} are possible, but so is the ``\isi{object} reading'' under which Mateja hit Meredith.
...
Examples of \is{ellipsis, verb-phrase}VPE with missing antecedents are also grammatical in \il{Ho-Chunk}Hocąk. In \REF{ex:johnson:51a}, the DP \emph{kšeexetehižą} `a pineapple' in the second conjunct is the antecedent for the null pronominal \isi{subject} of the verb \emph{sguu} `sweet'. In \REF{ex:johnson:51b}, the \is{verb phrase}VP containing the antecedent is elided, and the resulting sentence is grammatical. Like \ili{English}, a pronoun cannot find its antecedent in a negated \is{clauses}clause \REF{ex:johnson:52}.

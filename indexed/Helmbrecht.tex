...
The possessive relation in \REF{leg} includes a \isi{body part term} as \isi{possessed} noun (inseparable, inalienable) with a non-human \isi{possessor}. The example in \REF{wheel} represents a part-whole relation with an inanimate object as \isi{possessor} and an inanimate object as \isi{possessed} (separable, alienable). Both possessors in \REF{leg} and \REF{wheel} can be interpreted either as specific or as generic. 
...
\begin{table}
\caption{Alienable vs. inalienable distinction in \il{Ho-Chunk}Hoocąk} \label{alienabilityinhochunk} 
...
\begin{table}
\caption{In\isi{alienable possession} in \il{Apsaalooke}Crow (\citealt[52]{Graczyk2007})} \label{crowinalienablepossession}
\begin{tabular}{ l l l }
\lsptoprule
& stem \textit{apá}  & \\
\midrule	
...
However, there are also differences. For instance, the 2POSS forms do not trigger a shift in \isi{stress} assignment as in \il{Apsaalooke}Crow, and the inalienable possessive prefixes are true prefixes, whereas the corresponding alienable forms are analyzed as clitics.  The alienable forms are identical to the ones for \isi{inalienable possession} plus /\textit{ta-}/ which can be found in other Siouan languages as well (cf. e.g. in \ili{Lakota} alienable PCs of set\textsubscript{4} nouns which have a \textit{-t\textsuperscript{h}a} prefix added to the undergoer pronominal prefix; cf. \tabref{lakotaalienability} below). There is no mention of a depossessivizer in Boyle's grammar of \ili{Hidatsa}.
...
\midrule
\il{Apsaalooke}Crow & 1) part-whole & 1) body parts, kinship terms & rest, plus some exceptions &	Ø \\
